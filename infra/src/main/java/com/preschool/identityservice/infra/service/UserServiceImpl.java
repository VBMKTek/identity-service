...
public class UserServiceImpl implements UserDataAccessService {
    ...
    public UserData getUserByKeycloakId(String keycloakId) {
        ...
        UserEntity user =
                userRepository
                        .findByKeycloakIdWithRoles(keycloakId)
                        .orElseThrow(
                                () -> new RuntimeException("User not found with Keycloak ID: " + keycloakId));
        return userMapper.toData(user);
    }
    ...
    public List<UserData> getUsersByRole(UUID roleId) {
        ...
        List<UserEntity> filteredUsers =
                users.stream()
                        .filter(
                                user -> user.getRoles().stream().anyMatch(role -> role.getRoleId().equals(roleId)))
                        .toList();
